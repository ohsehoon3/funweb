...
<body>
	<div id="wrap">
		...
		<article>
			...
			<h1>파일 게시판 글수정</h1>
			<form action="fileModifyPro.do" method="post" name="frm" enctype="multipart/form-data" >
				...
				<table id="notice">
					...
					<tr>
						<th scope="col">글내용</th>
						<td style="text-align: left;">
							<textarea name="content" rows="13" cols="40">${ boardVo.content }</textarea>
						</td>
					</tr>
				</table>
	...
	<script>
		var fileCount = ${ fn:length(attachList) }; /* JSP 교재 p590 */
		...
		$('button#btnAddFile').click(function (event) {
			...
			//$(this).next().find('div#newFileBox').append(str); /* 아이디 없을때 사용 */
			fileCount++;
		});

		// 동적 이벤트 바인딩
		$('div#newFileBox').on('click', 'span.delete-addfile', function () {
			$(this).parent().remove(); /* empty()는 안쪽요소만 삭제 */
			fileCount--;
		});
